...
| `MCP_ALLOWED_HOSTS` | no | Only used when `MCP_TRANSPORT=http`; comma-separated list of hostnames the server will accept in the `Host` header (DNS-rebinding protection). Setting it **replaces** the loopback-only default, so include `localhost`/`127.0.0.1` if local clients or container healthchecks also need to connect. A reverse proxy forwards the public hostname, so it must be listed — e.g. `MCP_ALLOWED_HOSTS=mcp.example.com,localhost` |
