...
describe("runIncrementalSync", () => {
  ...
  it("sends modified__gte (inclusive) from the stored watermark on a subsequent pass", async () => {
    const store = await memoryStore(IDENTITY);
    ...
    // Inclusive, not strict -- see runIncrementalSync's own comment: a
    // strict `gt` would permanently drop any document sharing the exact
    // watermark timestamp with no secondary sort key to break the tie.
    expect(requestUrl.searchParams.get("modified__gte")).toBe("2024-01-01T00:00:00Z");
    expect(requestUrl.searchParams.get("modified__gt")).toBeNull();
    expect(requestUrl.searchParams.get("ordering")).toBe("-modified");
    store.close();
  });

  it("re-fetching a tied watermark boundary is a cheap no-op via the content-hash short-circuit", async () => {
    const store = await memoryStore(IDENTITY);
    const embeddingProvider = fakeEmbeddingProvider();
    const doc = { id: 5, modified: "2024-01-01T00:00:00Z", content: "same content" };
    const { client: client1 } = setup([documentsListRoute([doc])]);
    await runIncrementalSync({
      client: client1,
      store,
      embeddingProvider: embeddingProvider as never,
    ...
    // A second, sibling document shares the exact same `modified` value --
    // the kind of tie a bulk-edit request can produce. With `gte`, the next
    // pass re-fetches the boundary doc (id 5) alongside the new sibling
    // (id 6); doc 5's unchanged content hash makes it a cheap skip, and doc
    // 6 -- which a strict `gt` would have silently excluded forever -- gets
    // properly indexed.
    const sibling = { id: 6, modified: "2024-01-01T00:00:00Z", content: "different content" };
    const { client: client2 } = setup([documentsListRoute([sibling, doc])]);
    const summary = await runIncrementalSync({
      client: client2,
      store,
      embeddingProvider: embeddingProvider as never,
