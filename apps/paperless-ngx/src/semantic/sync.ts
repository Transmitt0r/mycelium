...
export async function runIncrementalSync(params: RunSyncParams): Promise<SyncSummary> {
  ...
  // `gte`, not `gt`: paperless can stamp the exact same `modified` value on
  // multiple documents (e.g. one bulk-tag-edit request touching many docs
  // at once). A strict `gt` would permanently exclude every document tied
  // with the watermark boundary once it's set -- there's no secondary sort
  // key to break the tie deterministically via the query alone. Re-fetching
  // the boundary timestamp's documents on every subsequent pass is cheap:
  // the content-hash short-circuit below turns anything actually unchanged
  // into a no-op skip rather than a wasted re-embed.
