...
| `MCP_BIND_HOST` | no | Only used when `MCP_TRANSPORT=http`; default `127.0.0.1` (loopback-only). Set to `0.0.0.0` to expose the server on all interfaces (e.g. a bridged Docker network) — always an explicit choice, never the default. The app provides **no authentication**; any non-loopback exposure MUST sit behind an authenticated reverse proxy (e.g. Caddy Basic auth) and ideally run with `TRILIUM_READ_ONLY=true` |
