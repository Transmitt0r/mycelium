...
| `MCP_ALLOWED_HOSTS` | no | Only used when `MCP_TRANSPORT=http`; comma-separated list of hostnames the server will accept in the `Host` header (DNS-rebinding protection). A reverse proxy forwards the public hostname, so it must be listed — e.g. `MCP_ALLOWED_HOSTS=mcp.example.com` |
