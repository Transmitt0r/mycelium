...
describe("readRange", () => {
  ...
  it("caps the range at MAX_RANGE_LINES from start_line", () => {
    ...
    const result = readRange(long, 1, 599);
    expect(result.end_line).toBe(500);
  });

  it("throws when end_line is before start_line", () => {
    expect(() => readRange(content, 5, 3)).toThrow(/end_line/);
  });

  it("returns an empty slice when start_line is past the end of the content", () => {
    const result = readRange(content, 20, undefined);
