...
export class SemanticIndexStore {
  ...
  static async open(indexPath: string, dimensions: number): Promise<OpenStoreResult> {
    ...
    // below can still see and close a `db` that opened successfully but
    // failed during a later step (e.g. ensureSchema hitting a disk-full or
    // incompatible-on-disk-schema error) -- a `const` scoped to the try
    // block would go out of reach in the catch, leaking the handle/lock.
    ...
    try {
      ...
      // Best-effort cleanup -- a close-time error here shouldn't mask or
      // replace the original failure being reported below.
      try {
        db?.close();
      } catch {
        // ignore
