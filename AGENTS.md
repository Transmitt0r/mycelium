...
- `apps/{paperless-ngx,trilium,onepassword}` — the OpenClaw plugins this toolkit was extracted from, merged into this repo via `git subtree` (full history preserved) so they can consume `@transmitt0r/mycelium-*` as `workspace:*` dependencies instead of a publish round-trip. Each still publishes to npm independently under its own name (`@transmitt0r/openclaw-plugin-*`), released via `semantic-release` + Conventional Commits — separate from this repo's Changesets flow, but (unlike before the merge) run from a single root-level matrix workflow (`.github/workflows/release-apps.yml`), not a per-app one: GitHub Actions only discovers workflows under the repo root's `.github/workflows/`, so a nested `apps/*/.github/workflows/*.yml` silently never runs. `semantic-release-monorepo` (each app's `.releaserc.json`) scopes commit analysis and the release tag to that app's own directory, since all three apps now share one git history.
...
- **ClawHub** — `apps/*` only. `core/*`/`tools/*` can't qualify: `@transmitt0r/mycelium-mcp` exists specifically so tools work *without* OpenClaw. Published automatically alongside npm as part of each app's own `semantic-release` run (`scripts/publish-to-clawhub.sh`, wired in as an `@semantic-release/exec` `publishCmd` — see each app's `.releaserc.json`/`CONTRIBUTING.md`), needs a `CLAWHUB_API_KEY` repo secret.
