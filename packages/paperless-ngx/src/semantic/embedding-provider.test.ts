...
// than vi.stubGlobal -- matches this repo's stubFetch pattern
// (documents.test.ts) in spirit, but injected explicitly since
// EmbeddingProviderHandle takes fetchImpl as a constructor option
// specifically so tests never have to touch the real global fetch.
...
describe("EmbeddingProviderHandle", () => {
  ...
  it("splits a batch larger than 100 items into multiple sequential requests", async () => {
    ...
    const fetchImpl = vi.fn<typeof fetch>(async (_input, init) => {
      ...
      return jsonResponse({
        embeddings: body.requests.map((_, i) => ({ values: [i] })),
      });
    });
    const handle = makeHandle(fetchImpl);
